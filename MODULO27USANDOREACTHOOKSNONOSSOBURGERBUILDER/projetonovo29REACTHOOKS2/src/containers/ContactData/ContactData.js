...
const ContactData = (props) =>  {
  ...
  const orderHandler = (event) => {
    ...
    const orderData = {
      ...
      postalCode: orderForm.zipCode.value, //
      country: orderForm.country.value, /// 
      deliveryMethod: orderForm.deliveryMethod.value, //
      userId: props.userId,
    };

    props.onOrderAttempt(orderData, props.token);
  };

  // const validInputChecker = (formElement) => {
  //   if (!formElement.valid) {
  //     return false;
  //   }
  // };
    ...
    if (props.redirectToHome && props.error) {
      console.log('test');
      alert(
        `An error has occurred: ${props.error}. Redirecting to Burger Builder...`
      );
      redirectToHome = <Redirect to="/" />;
    }
        ...
        <form
          onSubmit={(event) => {
            if (
              ...
              !orderForm.street.value ||
              !orderForm.zipCode.value ||
              !orderForm.email.value ||
              !orderForm.country.value
            ) {
              alert('Please enter your contact information!');
              return;
            }
